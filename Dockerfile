# Use the official Node.js 20 image as base (required for Sanity)
...
# Build the application (skip search data generation for now)
RUN cp next.config.docker.ts next.config.ts
ENV NEXT_PUBLIC_SANITY_PROJECT_ID=u9m27k7u
ENV NEXT_PUBLIC_SANITY_DATASET=production
ENV SKIP_SANITY_BUILD=true
RUN npm run build
